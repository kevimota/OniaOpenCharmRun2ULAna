...
class AnalyzerProcessor(processor.ProcessorABC):
   def __init__(self):
      ...
      mass_axis = hist.Bin("mass", r"$m_{\mu\mu}$ [GeV]", 1000, 0.25, 100)
   ...
   def process(self, df):
      ...
      #same vtx
      same_vtx = (np.logical_or(dimuons.i0['vtxIdx'] == dimuons.i1['vtxIdx'], np.absolute(dimuons.i0['z'] - dimuons.i1['z']) < 0.2))
